/* eslint-disable react/no-unstable-nested-components */
import React from 'react';
import {
  View, StyleSheet, FlatList, TouchableOpacity,
} from 'react-native';
import { ListItem, Avatar } from '@rneui/themed';
import { Divider } from 'react-native-paper';
import { useEffect, useState } from 'react';
import { setUserProperties } from 'firebase/analytics';
import { Text } from 'react-native-elements';
import { getUser } from '../src/firestore';
// import firestore from '@react-native-firebase/firestore';
// import Loading from '../components/Loading';

export default function HomeScreen({ navigation }) {
  const [users, setUsers] = useState(null);
  useEffect(() => {
    // we get all users using our firebase function and set the useState variable users to hold the data
    const getUsersFireBase = async () => {
      const data = await getUser();
      setUsers(data);
    };

    // we call the function that does this!
    getUsersFireBase();
  }, []);
  return (
    <View style={styles.container}>
      <FlatList
        data={users}// change this once i
        keyExtractor={(item) => item.data}
        ItemSeparatorComponent={() => <Divider />}
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => navigation.navigate('ChatScreen', { person: item })}>
            <ListItem bottomDivider>
              <Avatar
                size="medium"
                rounded
                source={{ uri: 'data:image/png;base64,' }}
              />
              <View style={styles.oval}><View style={{ justifyContent: 'center' }}><ListItem.Title style={styles.text}>{item.data.name}</ListItem.Title></View></View>
            </ListItem>
          </TouchableOpacity>
        )}
      />
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    backgroundColor: '#f5f5f5',
    flex: 1,
  },
  text: {
    textAlignVertical: 'center',
    textAlign: 'center',
  },
  listTitle: {
    fontSize: 22,
  },
  listDescription: {
    fontSize: 16,
  },
  avatar: {
    borderRadius: 10,
    width: 60,
    height: 60,
  },
  oval: {
    width: 250,
    borderRadius: 20,
    height: 60,
    backgroundColor: '#D5E0D8',
    justifyContent: 'flex-end',
    marginLeft: 37,
  },
  rectagle: {
    width: 100,
    borderRadius: 0,
    height: 60,
    backgroundColor: 'purple',
  },
  title: {
    fontSize: 15,
    alignContent: 'center',
  },
});
